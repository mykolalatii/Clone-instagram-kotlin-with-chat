...
import com.nikolai.finstagram.data.firebase.common.*
...
import com.nikolai.finstagram.common.*
...
class FirebaseUsersRepository : UsersRepository {
    ...
    override fun setOnlineStatus(status: String): Task<Unit> =
        database.child("users/${currentUid()!!}/online").setValue(status).toUnit()

    override fun getUserBy(username: String): LiveData<List<User>?> {
        val query = database.child("users").orderByChild("username").equalTo(username)
        return FirebaseLiveDataSingleEven(query).map { users ->
            users.children.map {
                it.asUser()!!
             } }
    }
